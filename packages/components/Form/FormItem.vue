<script setup>
import { computed, provide, inject, onMounted, onUnmounted, ref, nextTick, watch, reactive, watchEffect, toRefs, useSlots } from "vue";
import Schema from "async-validator";
import { isNil, get, size, filter, some, isString, isNumber, endsWith, keys, includes, map, cloneDeep, isArray } from "lodash-es";
...
const validateStatus = ref("init");
const errMsg = ref("");
const inputIds = ref([]);

const getValByProp = (target) => {
...
const itemRules = computed(() => {
  ...
  if (!isNil(required)) {
    const requiredRules = filter(
      ...
      (item) => includes(keys(item[0]), "required")
    );
...
function getTriggeredRules(trigger) {
  ...
  if (rules) {
    ...
    }).map(({ trigger: t, ...rule }) => rule);
  }
  return [];
}

async function doValidate(rules) {
